...
describe("compact surface", () => {
  ...
  it("maps fork companion views into the compact explorer", () => {
    openSidePanelView({ ...compact, view: "repository_graph" });
    expect(usePanelStore.getState().explorerTab).toBe("repository_graph");

    openSidePanelView({ ...compact, view: "ci" });
    expect(usePanelStore.getState().explorerTab).toBe("ci");
    expect(useWorkspaceLayoutStore.getState().layoutByWorkspace[WORKSPACE_KEY]).toBeUndefined();
  });
...
describe("non-compact with splits", () => {
  ...
  it("opens fork companion views in the dedicated side panel", () => {
    openSidePanelView({ ...wide, view: "repository_graph" });
    openSidePanelView({ ...wide, view: "ci" });

    const tabs = collectAllTabs(layout().root);
    const graphTab = tabs.find((tab) => tab.target.kind === "repository_graph");
    const ciTab = tabs.find((tab) => tab.target.kind === "branch_ci");
    expect(graphTab?.target).toEqual({ kind: "repository_graph" });
    expect(ciTab?.target).toEqual({ kind: "branch_ci" });
    expect(paneIdHolding(graphTab?.tabId as string)).toBe(sidePanelPaneId());
    expect(paneIdHolding(ciTab?.tabId as string)).toBe(sidePanelPaneId());
  });
